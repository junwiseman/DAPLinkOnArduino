...
void USBD_Init (void) {
  ...
  SIM->SCGC4   |=   SIM_SCGC4_USBFS_MASK;  /* Enable USBOTG clock        */
	
	USB0->CLK_RECOVER_IRC_EN |= USB_CLK_RECOVER_IRC_EN_IRC_EN_MASK;       /* enable the IRC48M clock */
	USB0->CLK_RECOVER_CTRL |= USB_CLK_RECOVER_CTRL_CLOCK_RECOVER_EN_MASK; /* enable the USB clock recovery tuning */
	
  SIM->SOPT2 |= SIM_SOPT2_USBSRC(1);   /* Select MCGPLLCLK as clock  */
