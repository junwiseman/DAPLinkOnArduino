...
int32_t uart_set_configuration(UART_Configuration *config)
{
    ...
    if ((config->DataBits < 8) || (config->DataBits > 9)) {
		...
		switch (config->Parity) {
			case 1: parity_enable = 1; parity_type = 1; data_bits++; break;  /* Parity Odd */
			case 2: parity_enable = 1; parity_type = 0; data_bits++; break;  /* Parity Even */
			default:
					break;
	  }
    ...
    UART->CTRL = ((parity_enable << 1) | (parity_type << 0));
		if(data_bits == 9) {
			  UART->CTRL |= LPUART_CTRL_M_MASK;
		}

    // stop bits
    UART->BAUD &= ~LPUART_BAUD_SBNS_MASK;
    UART->BAUD |= (stop_bits << LPUART_BAUD_SBNS_SHIFT);		
/*	
	  TODO: Baudrate calculation
...
int32_t uart_write_data (uint8_t *data, uint16_t size)
{
    ...
    if (!tx_in_progress) {
        ...
        UART->DATA = write_buffer.data[write_buffer.idx_out++];
...
void UART_RX_TX_IRQHandler(void)
{
    ...
    if (s1 & LPUART_STAT_RDRF_MASK) {
        if ((s1 & LPUART_STAT_NF_MASK) || (s1 & LPUART_STAT_FE_MASK)) {
            ...
            read_buffer.data[read_buffer.idx_in++] = UART->DATA;
